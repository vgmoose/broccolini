...
void printHistory(WebView* webView)
{
	...
	{
		printf("%c%d: %s\n", i == webView->historyIndex ? '*' : ' ', i,
			webView->history[i].c_str());
	}
	printf("=========\n");
}

Element* URLBar::makeURLBarButton(std::string iconPath,
	std::function<bool()> action)
{
	int iconSize = 30;
	Element* base = (new Element())->setTouchable(true)->setAction(action);

	base->width = BUTTON_SIZE;
	base->height = BUTTON_SIZE;

	base->child(createThemedIcon(iconPath + (themeIsTooDark ? "-light" : ""))
...
void URLBar::createURLBarElements()
{
	...
	child(makeURLBarButton("back", [this]()
		{
          ...
          return true; })->constrain(ALIGN_LEFT, sidePadding - 1));
	...
	forwardButton = makeURLBarButton("forward", [this]()
		{
                    ...
                    resetBar();
                    webView->needsLoad = true;
                    return true; })->constrain(ALIGN_LEFT, sidePadding / 3 + btnAndPadding);
	forwardButton->hidden = true;
	child(forwardButton);
	...
	child(makeURLBarButton("add", [this, mainDisplay]()
		{
          ...
          this->resetBar();
          return true; })->constrain(ALIGN_RIGHT, sidePadding / 3 + btnAndPadding + 1));
	...
	child(makeURLBarButton("tabs", [this, mainDisplay]()
		{
          ...
          return true; })->constrain(ALIGN_RIGHT, sidePadding));
	...
	bookmarkButton = makeURLBarButton("star", [this, mainDisplay]()
        ...
        auto favThumbPath =
            "./data/favorites/" + base64_encode(webView->url) + ".png";
        webView->screenshot(favThumbPath);
        // add the url to the favorites list
        mainDisplay->favorites.push_back(webView->url);
        return true; })->constrain(ALIGN_LEFT, sidePadding / 3 + btnAndPadding * 2 + 10);
	...
	child(makeURLBarButton("refresh", [this]()
		{
          auto webView = this->webView;
          webView->needsLoad = true;
          return true; })->constrain(ALIGN_RIGHT, sidePadding / 3 + btnAndPadding * 2 + 10));
}
...
bool URLBar::process(InputEvents* event)
{
	...
	if (event->touchIn(width / 2 - innerWidth / 2, height / 2 - innerHeight / 2,
			innerWidth, innerHeight))
	{
		...
		if (event->isTouchDrag())
		{
			...
			CST_roundedRectangleRGBA(
				RootDisplay::renderer, width / 2 - innerWidth / 2,
				height / 2 - innerHeight / 2, width / 2 + innerWidth / 2,
				height / 2 + innerHeight / 2, 15, 0x0, 0x0, 0x0, 0xff);
			return true;
		}
	}
	else if (event->isTouchDrag())
	{
		...
		{
			// we're dragging outside of the text-editable area, but still on the URL
			// bar, we can refresh
			return true;
		}
		return false;
...
void URLBar::render(Element* parent)
{
	...
	CST_SetDrawColorRGBA(RootDisplay::renderer, theme_color.r, theme_color.g,
		theme_color.b, 0xff);
	...
	if (mainDisplay->privateMode)
	{
		...
		CST_roundedBoxRGBA(
			RootDisplay::renderer, x + width / 2 - darkInnerWidth / 2,
			y + height / 2 - innerHeight / 2, x + width / 2 + darkInnerWidth / 2,
			y + height / 2 + innerHeight / 2, 15, 0x66, 0x66, 0x66, 0xff);
	}
	else
	{
		CST_roundedBoxRGBA(
			RootDisplay::renderer, x + width / 2 - innerWidth / 2,
			y + height / 2 - innerHeight / 2, x + width / 2 + innerWidth / 2,
			y + height / 2 + innerHeight / 2, 15, fmin(theme_color.r + 0x11, 0xff),
			fmin(theme_color.g + 0x11, 0xff), fmin(theme_color.b + 0x11, 0xff),
			0xff);
	}

	if (highlightingKeyboard)
	{
		CST_roundedBoxRGBA(RootDisplay::renderer, width / 2 - innerWidth / 2,
			height / 2 - innerHeight / 2, width / 2 + innerWidth / 2,
			height / 2 + innerHeight / 2, 15, 0xad, 0xd8, 0xe6,
			0x90);
		CST_roundedRectangleRGBA(
			RootDisplay::renderer, width / 2 - innerWidth / 2,
			height / 2 - innerHeight / 2, width / 2 + innerWidth / 2,
			height / 2 + innerHeight / 2, 15, 0x66, 0x7c, 0x89, 0x90);
	}
...
void URLBar::updateVisibleUrlText()
{
	...
	while (urlTextWidth > innerWidth)
	{
		...
		this->urlText->setText(
			this->currentUrl.substr(0, this->currentUrl.length() / 2 - trimOffset) + "…" + this->currentUrl.substr(this->currentUrl.length() / 2 + trimOffset));
		this->urlText->update(true);
...
std::string URLBar::getCurTabScreenshotPath()
{
	...
	std::string viewFolder = ((MainDisplay*)RootDisplay::mainDisplay)->privateMode ? "pviews"
																				   : "views";
